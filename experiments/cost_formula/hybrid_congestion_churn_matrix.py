...
from experiments.common.independence_stats import distance_correlation, permutation_test, spearman_rho, variance_inflation_factors
...
def load_real_samples() -> List[Tuple[float, float, float]]:
    ...
    if not samples:
        raise RuntimeError(
            "No real samples found -- run scripts/mininet/independence_check.py "
            "and scripts/mininet/loss_saturation_check.py first."
        )
    return samples
