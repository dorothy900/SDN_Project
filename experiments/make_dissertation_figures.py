#!/usr/bin/env python3
"""
Make Dissertation Figures - dense, multi-panel matplotlib figures for the
dissertation materials report, matching the visual density of this
project's earlier ad-hoc figures (e.g. the congestion.py sustained/chronic/
churn-vs-delay panel and the weight-search benchmark-hardening panel, both
produced in prior sessions but never committed as code -- see this
session's scenario design audit, finding F1, and its Addendum on
congestion.py's identical gap).

Companion to experiments/make_figures.py (which produces the simpler,
one-chart-per-file PNGs used in the Scenario Design Audit report). This
script produces the denser, multi-panel versions used in the dissertation
narrative report, from the same underlying CSVs already in results/.

Run as: python3 -m experiments.make_dissertation_figures
Writes PNGs to results/figures/dissertation/.
"""
...
OUTPUT_DIR = Path("results/figures/dissertation")
