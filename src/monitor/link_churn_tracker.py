...
class LinkChurnTracker:
    ...
    def has_changed_recently(self, link_id: str, within_seconds: float, now: Optional[float] = None) -> bool:
        ...
        the last `within_seconds`. Added 2026-08-19 so DelayJitterTracker
        can exclude a link's own post-switch settle window from its rolling
        jitter stats (see NetworkState.update_link_statistics) -- a switch
        event itself can cause a real, transient delay blip that has
        nothing to do with steady-state jitter, and would otherwise let
        delta (control-plane churn) and zeta (data-plane jitter) partly
        double-count the same underlying event.
